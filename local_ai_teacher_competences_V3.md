...
Continue is the VS Code extension that connects your editor to the local model. Create or
edit `~/.continue/config.yaml` (global) or `.continue/config.yaml` (per project) and paste:

``` yaml
schema: v1

models:
  - name: Gemma (Local — Private)
    provider: ollama
    model: gemma:latest
...
### Step 3 — Rules live in `config.yaml`

In Continue v1.x there is no separate `.continuerules` file. Project-level rules live
directly inside `config.yaml` under the `rules:` key — the *OER Workshop Assistant* rule
is already included in the config above.

To add a second, project-specific context rule, append another entry:

``` yaml
rules:
  - name: My project context
    rule: "This project builds a course on renewable energy for second-year students."
```
...
| Add selection to chat | select code → `Ctrl+L` / `Cmd+L` |
| Inline edit selection | select code → `Ctrl+I` / `Cmd+I` |
| Inline autocomplete | type, accept with `Tab` |
| Add a file to context | `@file my_lesson.md` in chat |
| Switch model | click the model name at the bottom of the sidebar |
...
In Continue chat (rules are active automatically via `config.yaml`), try this Vibe Coding prompt:
...
Then ask Continue to refine or audit its own output. Notice that it runs **completely
...
| 4 | **Continue** (VS Code extension) | Wires VS Code to the local Gemma model | VS Code Marketplace → "Continue" | Pre-seed `~/.continue/config.yaml` from this sheet. |
...
- [ ] **Pre-seed** `~/.continue/config.yaml` (copy from Section 5, Step 2).
